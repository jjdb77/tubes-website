...
- **De vragenlijst staat op een eigen adres**: `/production-finance-health-check/assessment/` (`src/health-check-assessment.njk`). Opzet: per stap een thema met een paar vragen, en als laatste stap het rapport. Nu drie thema's van drie vragen (planning en budgettering, uitgaven en controle, forecasting en rapportage) plus het rapport, dus vier stappen.
  ...
  - **De laatste stap is het rapport**, opgebouwd uit de eigen antwoorden: per thema een balkje met de verdeling, daarna "waar we mee beginnen" met hoogstens drie punten (eerst wat als kans is aangemerkt, dan wat beter kan), elk met de bevinding uit het databestand en een artikel om te lezen, en tot slot alle antwoorden uitklapbaar. Bewust **geen cijfer en geen oordeel**: dat kan pas uit het gesprek komen. De antwoorden worden verstuurd bij de sprong naar het rapport, dus ze staan vast vóór het in beeld komt.
