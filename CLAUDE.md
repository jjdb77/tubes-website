...
- **De vragenlijst staat op een eigen adres**: `/production-finance-health-check/assessment/` (`src/health-check-assessment.njk`). Opzet is **vier stappen**: drie keer twee vragen met elk een eigen thema (planning en budgettering, uitgaven en controle, forecasting en rapportage), en als vierde stap de samenvatting. Twee vragen per scherm in plaats van zes op een rij, zodat elke vraag aandacht krijgt.
  ...
  - **Stap 4 is de uitkomst**: de zes gebieden met de labels die de aanvrager zelf koos, en één zin over waar we mee beginnen (de gebieden die hij als Opportunity markeerde). Bewust **geen cijfer en geen oordeel**: dat kan pas uit het gesprek komen. De antwoorden worden verstuurd bij de overgang van stap 3 naar 4, dus ze staan vast vóór de samenvatting in beeld komt.
  - Elke vraag heeft ook een korte naam (`short` in de front matter) voor die zin: de volle labels bevatten zelf al een "and" en dat leest niet in een opsomming.
