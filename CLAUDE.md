...
- Data: `src/_data/filmlocations.json` (CMS: "Filmlocaties (locatiegids)"). Per locatie: id, name, country, region, lat/lng, type (**vaste lijst van 19** sinds 3-9-2026: naast de landschaps- en erfgoedtypen ook `House & apartment`, `Hospital, prison & institution`, `Office, school & public building`, `Bar, restaurant & shop` en `Theatre, arena & venue`, want de gids moet gewone productielocaties bevatten, niet alleen bezienswaardigheden), setting (1 zin), known_for, official_url, commission_url, `url` (pagina in de catalogus van een filmcommissie), `source_urls` (waar de gegevens vandaan komen; tonen als "Source 1, 2" in de vergelijking) en de praktische velden price_note/price_eur_day/capacity/facilities/suitability/permit_needed/logistics: **alleen invullen wat de locatie zelf publiceert**, leeg is beter dan een schatting. Geen maximum, groeit via CMS en suggesties.
- **Controleer de data met `node scripts/check-filmlocations.mjs`** (velden, uniek id, type uit de lijst, coördinaten binnen het land, geen em-dashes, vrije fotolicentie). Met `--net` haalt hij bovendien elke URL op en meldt wat niet laadt. Draai dit na elke grote toevoeging; het script accepteert ook een los JSON-bestand als eerste argument.
- De gids toont het **echte aantal** boven de lijst en laadt per 48 kaarten bij ("Show more"); kaart en telling gaan over alle treffers.
- **Omvang**: op 3 en 4-9-2026 uitgebreid van 63 naar **ruim 1.650 locaties** in dezelfde 21 landen, in vier onderzoeksrondes met agenten per land plus verrijkingsrondes voor de praktische velden. Verdeling: VK 199, Duitsland 137, Polen 122, Italie 118, Spanje 104, Nederland 101, Frankrijk 99, Hongarije 96, Tsjechie 94, Oostenrijk 79, daarna de rest tussen 22 en 72. Ruim 760 hebben een vrije foto, bijna 590 praktische gegevens en ruim 1.600 bronlinks. Verder zoeken naar foto's loont nauwelijks: de resterende locaties zijn juist gewone panden waar Wikipedia geen artikel over heeft, en een kaart zonder foto toont gewoon het typelabel. Doel was niet meer bezienswaardigheden maar **gewone productielocaties**: 'Theatre, arena & venue' (290), 'Industrial & derelict' (271), 'Office, school & public building' (215) en vervoer (197) zijn nu de grootste categorieen; paleizen staan op 30 en kastelen op 48. Aanleiding: een beoordeling die terecht stelde dat 63 locaties (3 per land, vooral beroemde plekken) te dun zijn om een zoekmachine te heten.
- **Controleer na elke ronde** met `check-filmlocations.mjs --net` (links), `check-coordinates.mjs` en `check-duplicates.mjs`. De linkcontrole over 1.670 entries vond 22 kapotte adressen, waarvan zeventien filmcommissies die zijn verhuisd of opgeheven; dat soort verval merk je alleen door het na te lopen.
- **Wat het snelst werkt bij uitbreiden**: niet losse websites afzoeken, maar de **catalogus van een filmcommissie met een detailpagina per locatie**, gevonden via haar sitemap. Bewezen productief: Vienna Film Commission, Silesia Film Commission (621 vermeldingen, met adres, stroom, water, parkeren en vergunning per locatie), Madrid Film Office, Portugal Film Commission (620), Italy for Movies (2.487, met scheda tecnica), Hellenic Film Commission, Zagreb Film Office, de Tsjechische commissie, Poznan, Lodz, Wroclaw, Brno, Oost-Bohemen, MDM, Hessenfilm, Norwegian Film Commission, Film Tampere, English Heritage en Cadw. De Hongaarse NFI-gids is een PDF (uitlezen met pdftotext). Geblokkeerd voor automatische verzoeken: National Trust, Historic Royal Palaces, IWM, Tate, Kew, Screen Scotland, Wales Screen, NI Screen. Zoekmachines zijn tijdens die sessie niet bruikbaar gebleken; werk met sitemaps en bekende domeinen.
- **Regels bij uitbreiden** staan in `docs/location-research-brief.md` (nieuwe locaties) en `docs/location-enrichment-brief.md` (praktische velden invullen). Kort: elke entry heeft een werkende `official_url` van de locatie zelf of van de beheerder, en voldoet aan minstens een van drie: de pagina publiceert filminformatie, er is een gedocumenteerde opnamegeschiedenis, of de plek staat in de publieke catalogus van een filmcommissie (die pagina in `url`). Nooit tekst, gegevens of foto's uit andere locatiedatabases overnemen; locations.filmfrance.net verbiedt geautomatiseerd gebruik expliciet. **Alleen invullen wat de locatie zelf publiceert**: leeg is beter dan een schatting, en een verzonnen capaciteit gaat een producent gebruiken om te plannen.
- **Gereedschap** (allemaal met `node scripts/...`):
  - `merge-locations.mjs <repo> <map>` voegt onderzoeksbestanden samen. Bestanden die `enrich-` heten of `studios-patch.json` zijn PATCHES (`{id, veld...}`), de rest zijn hele locaties. Dubbele id's en namen worden overgeslagen.
  - `src/_data/locations-dropped.json` houdt bewust geschrapte locaties weg (met de reden), anders zet de volgende samenvoeging ze zo terug uit het bronbestand.
  - `check-filmlocations.mjs [bestand] [--net]` controleert velden, unieke id's, type uit de lijst, coördinaten binnen het land, em-dashes en fotolicenties; met `--net` ook elke URL. Sites die elk bot-verzoek weigeren staan als 'note' apart.
  - `check-coordinates.mjs [bestand] [--km 20] [--fix]` trekt lat/lng na bij Nominatim (1 verzoek per seconde).
  - `check-duplicates.mjs [bestand]` meldt bijna-dubbele locaties (gedeelde woorden plus afstand). Twee agents vinden dezelfde plek soms onder een andere naam; een echte dubbel hoort in `locations-dropped.json`, anders komt hij terug.
  - `add-photos.mjs [bestand] [--out patchbestand]` zoekt vrije foto's op Wikimedia Commons **via het Wikipedia-artikel** van de plek: het artikel moet coördinaten binnen 3 km hebben en alle kenmerkende woorden uit de naam in de titel. Zonder die tweede eis pakt hij het artikel over de stad en krijgt een hotel een skylinefoto. Alleen vrije licenties.
- **De kaart dunt uit.** Uitgezoomd over Europa zouden duizend stippen op elkaar vallen; daarom tekent de kaart alleen wat in beeld valt en hoogstens een marker per rastervakje van 46 pixels (`CELL` in de sectie). Dat geeft een gespreide greep, en bij inzoomen valt hetzelfde gebied over meer vakjes zodat de rest vanzelf verschijnt. De regel eronder zegt hoeveel er van de locaties **in beeld** getekend zijn. Opnieuw tekenen gebeurt op `moveend` en `zoomend`.
- Zoeken werkt op losse woorden met een terugval op de beste treffers, dus een zoekvraag levert vrijwel altijd iets op; de regel eronder zegt eerlijk dat het een deeltreffer is. De filters tonen aantallen per land en per type.
- Let op de **paginaomvang**: 1.671 locaties is 3,6 MB HTML (436 kB gzip), want de kaarten staan in de HTML (goed voor SEO) en de praktische velden nog eens als JSON. Filteren zelf is geen probleem (1 tot 3 ms over 1.671 kaarten, gemeten): het zoekveld wacht 140 ms tot het typen stilvalt en de code schrijft `hidden` alleen als die verandert. Wordt de omvang wel een bezwaar, dan is de volgende stap alleen de eerste kaarten serverside renderen en de rest uit de JSON opbouwen, met het SEO-verlies als afweging.
