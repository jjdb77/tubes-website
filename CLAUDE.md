...
- **Het aanvraagformulier vraagt alleen het e-mailadres.** Daarna volgt meteen het bevestigingsscherm met de agenda. De vragen die daar eerst stonden (naam, bedrijf, rol, productietype) voelden dubbel omdat de vragenlijst er direct achteraan kwam; die staan nu alleen nog in de vragenlijst. Naam en telefoon vraagt 4Relations toch bij het boeken.
- Het formulier post naar **/api/health-check**: het aanvraagformulier stuurt `stage: "email"`, de vragenlijst `stage: "assessment"` met `lead_id` erbij. Zelfde JSONL-opslag en dezelfde beheerpagina als het contactformulier. /beheer laat de regel van stap 1 weg zodra stap 2 binnen is, en zet er een label bij. Gratis e-maildomeinen (gmail e.d.) worden niet geweigerd maar gemarkeerd.
