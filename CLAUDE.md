...
- **De vragenlijst staat op een eigen adres**: `/production-finance-health-check/assessment/` (`src/health-check-assessment.njk`). Opzet: **vier stappen**, drie thema's van drie vragen (planning en budgettering, uitgaven en controle, forecasting en rapportage) en het rapport als vierde. Bovenaan één segment per stap plus de teller "x of 9 answered". Rechts een paneel dat met de stap meewisselt. Alles-onder-elkaar is geprobeerd en weer teruggedraaid: het moeten stappen zijn, met het rapport als laatste.
