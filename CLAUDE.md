...
- **Werkt zonder account** (alles in localStorage), maar **opslaan vraagt om e-mail en wachtwoord**; inloggen kan vooraf, tussendoor of achteraf. Per account **één budget, hoogstens 10 versies, hoogstens 2000 regels** (constanten in server.js en als `data-max-*` op de sectie). "Save" overschrijft de open versie, "Save current as a new version" maakt een nieuwe (naam telt door: Draft 1 → Draft 2).
...
- **Formaten** (rekeningschema's) staan in `src/js/budget-templates.js` (`window.BudgetTemplates`): nu "Blank" en "Movie Magic" (41 secties, 337 accounts, gegenereerd uit `test/fixtures/magic-movie-template.xlsx` met `scripts/gen-budget-templates.mjs`). Joachim levert lijsten voor andere formaten aan; die worden objecten in hetzelfde bestand. Het voorbeeldbudget (`SAMPLE`) staat in budget-builder.js zelf.
