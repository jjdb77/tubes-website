...
- **AI-zoeken**: knop "Let AI turn this into filters" (of Enter) stuurt alleen de zoekvraag naar `/api/location-search`; server.js laat OpenAI er vaste filters van maken (terms/country/type, JSON), de database gaat nooit mee. Herkende filters staan als bewerkbare chips; zonder sleutel/limiet/fout valt alles terug op gewoon tekstzoeken. Sleutelbeheer op **/beheer/ai** (Basic Auth): write-only veld (alleen laatste 4 tekens zichtbaar), modelwhitelist, testknop (models.list), maandlimiet in euro's met verbruiksteller (ruwe bovengrens). Sleutel versleuteld (AES-256-GCM, afgeleid van `AI_SECRET` of `ADMIN_PASSWORD`) op de volume in `ai-config.json`; wachtwoordwissel = sleutel opnieuw invoeren. Alternatief: env `OPENAI_API_KEY`. **Nooit een sleutel in de repo of het CMS** (repo is publiek). Rate-limit 10/10min/IP. Staat zo ook in de privacyverklaring (zoekvraag naar OpenAI).
