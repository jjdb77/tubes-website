...
- **Het einddoel is de agenda.** Na de aanvraag is de eerste knop "Pick a time" naar `/book-a-call/health-check/`, en het rapport van de vragenlijst eindigt met dezelfde knop. Wie liever voorstellen per mail wil, kan dat; dat staat eronder.
  - ⚠️ Die pagina stuurt nu door naar de **algemene** agenda (`booking_url`, lege slug), omdat het afspraaktype nog gemaakt moet worden in de nieuwe 4Relations-omgeving voor Tubes. Zodra die link er is: CMS > Site-instellingen > Boekingslinks per plan, bij het pad `health-check/` de slug invullen (en zo nodig een eigen `booking_url`, zoals de Enterprise-regel al doet). `book-a-call.njk` vangt een lege slug af.
