...
(function () {
  ...
  function detectMapping(rows, hasHeader) {
    ...
    if (hasHeader) {
      ...
      m.amount = find(/^(total|totaal|amount|bedrag|budget|subtotal|sum|cost|value|estimate|approved|current|revised)/, [m.group, m.code, m.desc]);
  ...
  const changesOnly = root.querySelector("[data-bc-changes-only]");
  let current = null;
  ...
  function useSheet(v, index) {
    ...
    if (!v.headerRow && v.mapping.amount >= 0 && v.rows.length > 1 && !looksNumeric(v.rows[0][v.mapping.amount]) && String(v.rows[0][v.mapping.amount]).trim()) {
      v.headerRow = true;
  ...
  function render() {
    ...
    const showAll = !changesOnly.checked;
    ...
    const body = r.groups.map((g) => {
      if (!showAll && g.changes === 0) return "";
      const rowsHtml = g.rows
        .filter((row) => showAll || row.status !== "same")
        .map((row) => `<tr class="bc-row bc-row-${row.status}">
        ...
        .join("");
      const groupName = g.name || (r.groups.length === 1 ? "All lines" : "Other lines");
      return `<tbody class="bc-group">
        <tr class="bc-group-row"><th scope="rowgroup" colspan="3">${esc(groupName)}</th><td class="bc-num">${money.format(g.a)}</td><td class="bc-num">${money.format(g.b)}</td><td class="bc-num bc-delta">${Math.abs(g.delta) < 0.005 ? "" : signed.format(g.delta)}</td><td class="bc-col-pct"></td></tr>
        ${rowsHtml}
      </tbody>`;
    }).join("");
    ...
    tableWrap.innerHTML = changes === 0 && !showAll
      ? `<p class="bc-empty">The two versions have the same amounts on every matched line. Untick "Only show changes" to see all ${r.rows.length} lines.</p>`
      : `<table class="bc-table">${head}${body}${foot}</table>`;
    ...
    notes.innerHTML = `Lines matched on ${matched}.${skipped.length ? " " + skipped.join("; ") + "." : ""} Amounts are shown as they appear in your sheets, without currency.`;
  }
  ...
  changesOnly.addEventListener("change", () => { if (current) render(); });
